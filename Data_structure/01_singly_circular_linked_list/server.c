...
list_t create_list(void)
{
 ...
 status_t  insert_end(list_t list, data_t data)
 {
   ...
   p_prev = get_p_prev(list, list);

   generic_insert(p_prev, new_node, list);
   return(success);
 }
 ...
 status_t  insert_before(list_t list, data_t e_data, data_t data)
 {
   ...
   p_prev = get_p_prev(list, e_node);
   generic_insert(p_prev, new_node, e_node);
   return(success);
 }
 ...
 status_t concat_list_m(list_t list1, p_list_t p_list2)
 {
   ...
   p_node_t p_prev1 = NULL;
   p_node_t p_prev2 = NULL;
   ...
   p_prev1 = get_p_prev(list1, list1);
   p_prev2 = get_p_prev(list2, list2);
   // connecting list1 and list2

   p_prev1->next = list2->next;        //joining list1 last node to first node of list2
   p_prev2->next = list1;              //joining list2 last node to head node of list1
 ...
 list_t  reverse_list_imm(list_t list)
 {
   ...
   p_run = get_p_prev(list, list);      // last node of the list
   while(p_run != list)
   {
      insert_end(reversed_list, p_run->data);
      p_run = get_p_prev(list, p_run);
   }
 ...
 status_t reverse_list_m(list_t list)
 {
   ...
   p_last_node = get_p_prev(list, list);
   ...
   while(list->next != p_last_node)
   {
      ...
      list->next = current_node->next;                // removing current node from list
...
*/
 ...
 static p_node_t get_p_prev(list_t list, p_node_t node)
 {
   p_node_t p_prev = NULL;

   p_prev = list;
   while(p_prev->next != node)
   {
      p_prev = p_prev->next;
   }
 ...
 static void  generic_delete(list_t list, p_node_t p_delete)
 {
   p_node_t p_prev = NULL;

   p_prev = get_p_prev(list, p_delete);
